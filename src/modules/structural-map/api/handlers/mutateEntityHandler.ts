...
import type { CreateEntityInput, CreateEntityResult } from '@/modules/structural-map/domain/types/PortfolioTypes';
...
export async function createEntityHandler(req: Request): Promise<NextResponse> {
  ...
  const result: CreateEntityResult = await repo.createEntity(body as CreateEntityInput);
